...
export default function ViolationsReportScreen() {
  const { data, loading, error, refresh } = useApi(getViolationsReport);
    ...
    <ScreenContainer>
      <View style={styles.statRow}>
        <Card style={styles.statCard}>
          <Text style={styles.statValue}>{data.summary.issuedLast30Days}</Text>
          <Text style={styles.statLabel}>Issued (30d)</Text>
        </Card>
        <Card style={styles.statCard}>
      ...
      <Card style={styles.statCard}>
        <Text style={styles.statValue}>{data.summary.pendingAppeals}</Text>
      ...
      <Text style={styles.sectionTitle}>Appeals By Status</Text>
      <ListCard>
        {data.appeals.byStatus.map((s) => (
          <ListRow key={s.status} title={titleCase(s.status)} right={<Text style={styles.count}>{s.count}</Text>} />
        ))}
      </ListCard>
    </ScreenContainer>
