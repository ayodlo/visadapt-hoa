...
**The application lives in the [`nextjs/`](nextjs/) directory.** All commands below are run from there. `nextjs/DEVLOG.md` is the running engineering log.
...
| `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` / `AWS_S3_BUCKET` | No | An S3 helper module exists (`nextjs/lib/s3.ts`) but **no feature currently uploads files** — see [Documents](#documents) |
...
- An S3 client helper (`nextjs/lib/s3.ts`, presigned GET/PUT) is implemented but **not called by any route**. Wiring real uploads is a deferred feature.
...
### Users & Profile
- **Users page** (staff only; server-side guard redirects residents): list all users. Admins can change roles and delete users; board members see a read-only roster.
...
Notable absences: no length caps on announcement/issue/legacy-maintenance bodies or document titles; no file-size limits anywhere (because there are no file uploads); no rate or frequency limits on content submissions (a resident can file unlimited issues/requests — only the auth endpoints are rate-limited).
...
- **Route protection:** middleware (`nextjs/proxy.ts`) verifies the JWT for every page and API path except `/login`, `/register`, `/forgot-password`, `/reset-password`, and `/api/auth/*`, redirecting to login. Every API handler *additionally* checks the session and role — defense in depth.
- **Rate limiting (auth endpoints):** per-IP fixed windows — login 20/15 min, register 10/hr, forgot-password 5/15 min, reset-password 10/15 min; excess requests get 429 with `Retry-After`. ⚠️ The limiter is in-memory (`nextjs/lib/rate-limit.ts`): a real global limit on a single Node server, but per-instance on serverless — treat it as brute-force friction there, and move to a shared store (e.g. Upstash Redis) for hard guarantees.
...
| Method | Path | Notes |
|---|---|---|
| POST | `/api/auth/register` | Open registration; always creates RESIDENT; auto-login (sets cookie); rate-limited 10/hr per IP |
| POST | `/api/auth/login` | Sets JWT cookie; rate-limited 20/15 min per IP |
| POST | `/api/auth/logout` | Clears cookie |
| GET | `/api/auth/me` | Current session user |
| POST | `/api/auth/forgot-password` | Always 200; emails reset link if Resend configured; rate-limited 5/15 min per IP |
| POST | `/api/auth/reset-password` | Consumes single-use token (1 h expiry); rate-limited 10/15 min per IP |
...
```bash
cd nextjs
npm test              # 106 Vitest unit tests (pure functions — no DB needed)
npm run test:e2e      # 27 Playwright e2e tests (needs DATABASE_URL + seeded DB; auto-starts dev server)
...
### UI/theming
Dark mode is the default; a sun/moon toggle (sidebar, mobile top bar, auth pages) persists the choice in `localStorage` with no flash on load. The entire dark theme lives in `nextjs/app/globals.css` via Tailwind v4 CSS-variable remapping — do **not** add `dark:` variants to pages, and never use `bg-gray-800/900` for "always dark" surfaces (grays invert); use `slate` instead.
...
- **Prisma CLI does not read `.env.local`** — export `DATABASE_URL` before running `npx prisma …` commands locally.
...
- The seed is idempotent but destructive (delete-then-recreate). `nextjs/DEVLOG.md` is the running engineering log.

## Repository history

The project began as a Vite + React client with an Express API (`client/` and `server/`, since removed) and was migrated to a single Next.js app in `nextjs/`.
