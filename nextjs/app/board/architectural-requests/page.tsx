...
export default function BoardArchRequestsPage() {
  ...
  const fetchList = useCallback(async (s: string, st: string, ty: string, pg: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/board/architectural-requests?${params}`);
  ...
  async function openDetail(id: string) {
    ...
    setSelected(null);
    try {
      const res = await fetch(`/api/board/architectural-requests/${id}`);
      if (!res.ok) throw new Error();
      const json = await res.json();
  ...
  async function saveChanges() {
    ...
    try {
      ...
      const res = await fetch(`/api/board/architectural-requests/${selected.id}`, {
        method: 'PATCH',
  ...
  // Board member can only set these statuses
  const boardStatusOptions = REQUEST_STATUSES.filter((s) =>
    ['UNDER_REVIEW', 'NEEDS_MORE_INFORMATION', 'APPROVED', 'DENIED'].includes(s.value)
  );
    ...
    <div className="flex h-full min-h-0 gap-0">
      ...
      <div className={`flex flex-col min-h-0 ${selected ? 'hidden lg:flex lg:w-80 xl:w-96 flex-shrink-0 border-r border-gray-200' : 'flex-1'}`}>
        <div className="flex-shrink-0 px-5 pt-5 pb-4 border-b border-gray-200">
          <PageHeader title="Architectural Review Queue" subtitle={`${total} requests`} />
        ...
        {totalPages > 1 && (
          <div className="flex-shrink-0 flex items-center justify-between px-5 py-3 border-t border-gray-200 text-sm">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="text-blue-600 disabled:text-gray-300 font-medium focus:outline-none">Previous</button>
            <span className="text-gray-500">{page} / {totalPages}</span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="text-blue-600 disabled:text-gray-300 font-medium focus:outline-none">Next</button>
          </div>
        )}
      </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-2xl mx-auto px-5 py-5 space-y-6">
            <div className="flex items-center gap-3">
              <button onClick={() => setSelected(null)} className="lg:hidden text-sm text-blue-600 hover:text-blue-800 font-medium focus:outline-none">← Back</button>
              <div className="flex-1 flex items-center gap-2 flex-wrap">
                <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-medium">{requestTypeLabel(selected.requestType)}</span>
            ...
            {!['APPROVED', 'DENIED', 'WITHDRAWN', 'DRAFT'].includes(selected.status) && (
              <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                <h3 className="text-sm font-semibold text-gray-900">Board Review</h3>

                {/* Quick decision buttons */}
                <div className="flex gap-2 flex-wrap">
                  <button
                    onClick={() => setNewStatus('APPROVED')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 ${newStatus === 'APPROVED' ? 'bg-green-600 text-white border-green-600' : 'border-green-300 text-green-700 hover:bg-green-50'}`}
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => setNewStatus('DENIED')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 ${newStatus === 'DENIED' ? 'bg-red-600 text-white border-red-600' : 'border-red-300 text-red-700 hover:bg-red-50'}`}
                  >
                    Deny
                  </button>
                  <button
                    onClick={() => setNewStatus('NEEDS_MORE_INFORMATION')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500 ${newStatus === 'NEEDS_MORE_INFORMATION' ? 'bg-amber-500 text-white border-amber-500' : 'border-amber-300 text-amber-700 hover:bg-amber-50'}`}
                  >
                    Request More Info
                  </button>
                  <button
                    onClick={() => setNewStatus('UNDER_REVIEW')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 ${newStatus === 'UNDER_REVIEW' ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600 hover:bg-gray-50'}`}
                  >
                    Mark Under Review
                  </button>
                </div>
                ...
                {newStatus && (
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">
                      {newStatus === 'NEEDS_MORE_INFORMATION' ? 'What information is needed?' : 'Decision Reason'}
                      {(newStatus === 'APPROVED' || newStatus === 'DENIED') && <span className="text-red-500 ml-1">*</span>}
                    </label>
                    <textarea
                      value={decisionReason}
                      onChange={(e) => setDecisionReason(e.target.value)}
                      rows={3}
                      placeholder={newStatus === 'NEEDS_MORE_INFORMATION' ? 'Describe what additional information or documentation is required…' : 'Provide the reason for this decision…'}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    placeholder="Add a note for the resident or an internal board note…"
                ...
                <div className="flex gap-3">
                  ...
                  {newStatus && (
                    <button onClick={() => setNewStatus('')} className="px-4 py-2 text-sm text-gray-500 hover:text-gray-700 focus:outline-none">
                      Clear
                    </button>
                  )}
                </div>
            ...
            {/* Select different status for decided requests */}
            {['APPROVED', 'DENIED'].includes(selected.status) && (
              <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
                <h3 className="text-sm font-semibold text-gray-900">Override Decision</h3>
                <div className="flex gap-2">
                  {boardStatusOptions.filter((s) => s.value !== selected.status).map((s) => (
                    <button
                      key={s.value}
                      onClick={() => setNewStatus(s.value)}
                      className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors focus:outline-none ${newStatus === s.value ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600 hover:bg-gray-50'}`}
                    >
                      {s.label}
                    </button>
                  ))}
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        {c.author.role !== 'RESIDENT' && (
                          <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-medium">
                            {c.author.role === 'BOARD_MEMBER' ? 'Board' : 'Staff'}
                          </span>
                        )}
                        {c.isInternal && (
      ...
      <ConfirmDialog
        ...
        description={confirmDecision === 'APPROVED'
          ? 'Are you sure you want to approve this architectural request?'
          : 'Are you sure you want to deny this request? Please ensure you have provided a clear reason.'}
