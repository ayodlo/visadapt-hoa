...
export function AnnouncementsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['announcements', page, debouncedSearch],
    queryFn: () => {
      const params = new URLSearchParams({ page: String(page), limit: '10' });
      if (debouncedSearch) params.set('search', debouncedSearch);
      return apiClient.get<AnnouncementsResponse>(`/api/announcements?${params}`);
    },
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {isError && (
        <p className="text-sm text-red-500 text-center py-12">Failed to load announcements.</p>
      )}
      {data?.announcements.length === 0 && (
        <p className="text-sm text-gray-400 text-center py-12">
          {debouncedSearch ? 'No announcements match your search.' : `No announcements yet.${canPost ? ' Create the first one!' : ''}`}
        </p>
      )}
